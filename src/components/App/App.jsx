...
getFilmsList = (filmName, pageNumber = 1) => {
  api.getFilmsList(filmName, pageNumber).then((data) => {
    this.setState({ filmData: data.results,
      totalResults: data.total_results,
      isLoading: false,
      totalPages: data.total_pages,
      currentQuery: filmName, ratedList:[],
      totalRatedPages:null}) 
  ...
  render() {
    ...
    if(this.state.isRatedPage) return (
    ...
    return (
      ...
      <main>
      <GenresContext.Provider value={{genres: this.state.genres, addRating: this.addRating}}>
        <FilmsList filmData={this.state.filmData} isLoading={this.state.isLoading} isError={this.state.error} totalResults={this.state.totalResults} totalPages={this.state.totalPages} isRatedPage={this.state.isRatedPage} />
        </GenresContext.Provider>
      </main>
      <footer>
